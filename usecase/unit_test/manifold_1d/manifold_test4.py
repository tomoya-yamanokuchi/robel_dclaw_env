...
class Test:
    ...
    def run(self, task_space_value):
        numpy_unit_direction_vector = self.numpy_task_space.debug_task2any(TaskSpaceValueObject(NTD(task_space_value)))[0]
        torch_unit_direction_vector = self.torch_task_space.debug_task2any(TaskSpaceValueObject(NTD(to_tensor_double(task_space_value))))[0]

        np.testing.assert_almost_equal(
            actual  = numpy_unit_direction_vector,
            desired = to_numpy(torch_unit_direction_vector),
            decimal = 13
        )

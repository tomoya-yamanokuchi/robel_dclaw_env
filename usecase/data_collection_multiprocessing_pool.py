...
def rollout(constant_setting, ctrl_index, ctrl_task_diff):
    '''
    マルチプロセッシングで実行する関数
    '''
    ...
    for n in range(num_batch):
        repository.open(filename='domain{}_action{}'.format(ctrl_index, n))
...
class Demo_task_space:
    def run(self, config):
        ...
        proc_time = multiproc.run_Pool_from_chunked_ctrl(
            function         = rollout,

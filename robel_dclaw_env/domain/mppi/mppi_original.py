...
class MPPI(Controller):
    ...
    def obtain_sol(self, curr_x, g_xs):
        ...
        """
        # get noised inputs
        noise         = np.random.normal(loc=0, scale=1.0, size=(self.pop_size, self.pred_len, self.input_size)) * self.noise_sigma
        noised_inputs = noise.copy()

        for t in range(self.pred_len):
            if t > 0:
                noised_inputs[:, t, :] = self.beta * (self.prev_sol[t, :] + noise[:, t, :]) + (1 - self.beta) * noised_inputs[:, t-1, :]
            else:
                noised_inputs[:, t, :] = self.beta * (self.prev_sol[t, :] + noise[:, t, :]) + (1 - self.beta) * self.history_u[-1]


        noised_inputs   = np.clip(noised_inputs, self.input_lower_bounds, self.input_upper_bounds) # clip actions
        costs           = self.calc_cost(curr_x, noised_inputs, g_xs); rewards = -costs # calc cost

...
class EnvironmentMultiprocessing:
    ...
    def run_from_chunked_ctrl(self, function, constant_setting: EnvironmentConstantSetting, chunked_input: list):
        assert isinstance(constant_setting,  EnvironmentConstantSetting)
        assert type(chunked_input) == list
        start_time  = time.time()
        max_process = min(len(chunked_input), int(os.cpu_count()/3))
        ...
        proc_unit_chunked_ctrl = np.array_split(chunked_input, max_process)
        for ctrl_index, unit_chunked_input in enumerate(proc_unit_chunked_ctrl):
            # import ipdb; ipdb.set_trace()
            queue_input.put((ctrl_index, unit_chunked_input))
